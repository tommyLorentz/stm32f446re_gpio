...
1. Properties -> C/C++ Build - Settings -> MCU G++ Linker - Miscellaneous -> type "-specs=rdimon.specs -lc -lrdimon"

2. Debug configuration -> startup -> Run commands -> type "monitor arm semihosting enable"

3. In main.c "extern void initialise_monitor_handles();"

4. In main.c -> main() {"extern void initialise_monitor_handles();"}

5. Exclude resource from build -> sysmem.c
 */
